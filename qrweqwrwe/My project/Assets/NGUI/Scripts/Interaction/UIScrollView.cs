...
public class UIScrollView : MonoBehaviour
{
    ...
    public virtual void SetDragAmount(float x, float y, bool updateScrollbars)
    {
        ...
        //위젯의 bounds 받아옴
        Bounds b = bounds;
        //만약 bounds의 크기가 0인경우 리턴
        if (b.min.x == b.max.x || b.min.y == b.max.y) return;

        //오프셋을 고려한 최종 클리핑 영역. XY = Center, ZW = Size
        Vector4 clip = mPanel.finalClipRegion;
        ...
        //왼쪽 = 위젯 왼쪽 오프셋 + (클리핑 영역 너비/2)
        //오른쪽 = 위젯 오른쪽 오프셋 - (클리핑 영역 너비/2)
        //아래 = 위젯 아래 오프셋 + (클리핑 영역 높이/2)
        //위 = 위젯 위 오프셋 + (클리핑 영역 높이/2)
        ...
        {
            //현재 로컬 포지션 받아옴
            //수평이동 가능한 경우 현재 위치에 (클리핑 값 - 수평 선형보간값) 
            //수직이동 가능한 경우 현재 위치에 (클리핑 값 - 수직 선형보간값)
            //현재 로컬 포지션에 변경값 반영;
    ...
    void RemoveSpringPanel()
    {
        var springPanel = GetComponent<SpringPanel>();
        if (springPanel)
        {
            Destroy(springPanel);
        }
