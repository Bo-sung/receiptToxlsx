...
public class AdvancedIronSourceDependenciesManager : EditorWindow
{
    [MenuItem("IronSource/Integration Manager (Advenced)", false, 5)]
    public static void SdkManagerProd()
    {
        ShowISDependenciesManager();
    }
    ...
    private IEnumerator GetVersions()
    {
        ...
        {
            RefreshVersion(unityWebRequest.downloadHandler.text);
        }
    }

    private void RefreshVersion(string json)
    {
        savedJson = json;
    ...
    {
        var win = GetWindowWithRect<AdvancedIronSourceDependenciesManager>(new Rect(0, 0, Width, Height), true);
    ...
    private void ShowJsonMenu()
    {
        ...
        menu.AddItem(new GUIContent("Version 변경"), false, ShowJsonEditorWizard);

        menu.ShowAsContext();
    }

    private void ShowJsonEditorWizard()
    {
        JsonEditorWizard wizard = ScriptableWizard.DisplayWizard<JsonEditorWizard>("버전변경");
        wizard.text = savedJson;

        wizard.minSize = new Vector2(640f, 960f);
        wizard.maxSize = new Vector2(720, 1280f);
        wizard.Focus();
        wizard.Repaint();
        wizard.Show();
    }

    public class JsonEditorWizard : ScriptableWizard
    {
        ...
        void OnWizardCreate()
        {
            GetWindow<AdvancedIronSourceDependenciesManager>().RefreshVersion(text);
        }
    }
}
