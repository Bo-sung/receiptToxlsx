...
public class SuperScrollListWrapper : MonoBehaviour
{
    ...
    public delegate void OnItemPress(GameObject go, int dataIndex, bool press);
    ...
    public void SetPressCallback(OnItemPress pressCallback)
    {
    ...
    public void Resize(int dataSize)
    {
        ...
        ScrollView.RestrictWithinBounds(true, ScrollView.canMoveHorizontally, ScrollView.canMoveVertically); // SpringPanel을 생성 안함, 드래그시 자동 생성
    }

    public void RemoveSpringPanel()
    {
        var springPanel = ScrollView.GetComponent<SpringPanel>();
        if (springPanel)
        {
            Destroy(springPanel);
        }
    ...
    private void InitItems(GameObject prefab)
    {
        ...
        for (int i = 0; i < itemSize; i++)
        {
            ...
            if (go.GetComponent<Collider>() != null || go.GetComponent<Collider2D>() != null)
            {
                ...
                UIEventListener.Get(go).onPress = OnPress_Item;
                UIEventListener.Get(go).onDragStart = OnDragStart; // 드래그가 시작되면 프레스 종료
            }
        }
    ...
    private void OnPress_Item(GameObject go, bool isPress)
    {
        if (onItemPress == null)
        {
            return;
        ...
        if (_firstLine != null && _firstLine.index >= 0)
        {
            for (int i = 0; i < _firstLine.activeCount; i++)
            {
                if (_firstLine.members[i].gameObject == go)
                {
                    onItemPress(go, _firstLine.index * LineCapacity + i, isPress);
                    return;
                }
            }
        }
        if (_lastLine != null && _lastLine.index >= 0)
        {
            for (int i = 0; i < _lastLine.activeCount; i++)
            {
                if (_lastLine.members[i].gameObject == go)
                {
                    onItemPress(go, _lastLine.index * LineCapacity + i, isPress);
                    return;
                }
            }
        }
        for (int i = 0; i < _scrollLines.Count; i++)
        {
            ...
            for (int j = 0; j < line.activeCount; j++)
            {
                if (line.members[j].gameObject == go)
                {
                    onItemPress(go, line.index * LineCapacity + j, isPress);
                    return;
                }
            }
        }
    ...
    {
        OnPress_Item(go, false); // 드래그 시작할 때, 프레스 종료
    }
    ...
    public OnItemPress onItemPress;
