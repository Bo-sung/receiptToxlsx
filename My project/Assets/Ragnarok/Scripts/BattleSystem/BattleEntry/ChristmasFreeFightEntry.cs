...
namespace Ragnarok
{
    public sealed class ChristmasFreeFightEntry : BattleEntry
    {
        private const string SCENE_NAME = "TempSpecialMaze_Winter";
        private const string BGM = "Lutie_field";
        ...
        private readonly CharacterEntity clonePlayer;
        private readonly BetterList<ChristmasSkillData> skillList;
        ...
        public ChristmasFreeFightEntry() : base(BattleMode.ChristmasFreeFight)
        {
            sharingModel = player.Sharing;
            dungeonModel = player.Dungeon;

            skillDataRepo = SkillDataManager.Instance;
            totalRound = BasisType.EVENT_FF_ROUNT_COUNT.GetInt();
            config = FreeFightConfig.CHRISTMAS;

            playerBotPool = new PlayerBotEntityPool();
            battleTrapPool = new BattleTrapPool(BattleTrapType.ChristmasFreeFight);
            battleItemPool = new BattleItemPool();
            intBuffer = new Buffer<int>();
            clonePlayer = CharacterEntity.Factory.CreatePlayerClone(UnitEntity.UnitState.GVG);
            skillList = new BetterList<ChristmasSkillData>();
        }
        ...
        protected override void AddProtocalEvent()
        {
            ...
            Protocol.RECEIVE_FF_EVENTROOM_JOIN.AddEvent(OnUserJoin);
            ...
            Protocol.REQUEST_FF_ACTIVEBUFSKILL.AddEvent(OnActiveBuff);
            ...
            Protocol.RECEIVE_FF_FREEZE.AddEvent(OnUserFreezeStart);
            Protocol.RECEIVE_FF_FREEZE_END.AddEvent(OnUserFreezeEnd);
            Protocol.RECEIVE_FF_SHIELD.AddEvent(OnUserShieldStart);
            Protocol.RECEIVE_FF_SHIELD_END.AddEvent(OnUserShieldEnd);
            Protocol.REQUEST_FF_ATTACK.AddEvent(OnAttack);
        }

        protected override void RemoveProtocolEvent()
        {
            base.RemoveProtocolEvent();
            Protocol.RECEIVE_FF_EVENTROOM_JOIN.RemoveEvent(OnUserJoin);
            ...
            Protocol.REQUEST_FF_ACTIVEBUFSKILL.RemoveEvent(OnActiveBuff);
            ...
            Protocol.RECEIVE_FF_FREEZE.RemoveEvent(OnUserFreezeStart);
            Protocol.RECEIVE_FF_FREEZE_END.RemoveEvent(OnUserFreezeEnd);
            Protocol.RECEIVE_FF_SHIELD.RemoveEvent(OnUserShieldStart);
            Protocol.RECEIVE_FF_SHIELD_END.RemoveEvent(OnUserShieldEnd);
            Protocol.REQUEST_FF_ATTACK.RemoveEvent(OnAttack);
        }
        ...
        protected override void AddEvent()
        {
            ...
            clonePlayer.OnUseSkill += RequestSkillMotion;
            clonePlayer.OnApplySkill += RequestSkill;
        }
        ...
        protected override void InitCanvas()
        {
            ...
            uiBattleFreeFight.SetFreeFightEventType(FreeFightEventType.Christmas);
        ...
        public override void EndBattle()
        {
            ...
            clonePlayer.SetForceStatus(ForceStatusType.BasicStatusModeOff);
            clonePlayer.SetForceStatus(ForceStatusType.RefSkillOn);
            clonePlayer.ResetExtraOption(ExtraBattleOptionType.MaxHp); // MaxHp 옵션 제거
            clonePlayer.ResetExtraOption(ExtraBattleOptionType.AttackSpeed);
            clonePlayer.ResetExtraOption(ExtraBattleOptionType.MoveSpeed);
            clonePlayer.SetIgnoreWeapon(false);
            clonePlayer.SetIsFreeFightSkillCool(false);
            clonePlayer.SetForceStatus(ForceStatusType.BasicAttackSkillOn);
            clonePlayer.SetIgnoreTarget(false); // 무시타겟 설정 제거
        ...
        public override IEnumerator<float> YieldExitBattle()
        {
            ...
            Task<Response> task = Protocol.REQUEST_FF_EVENTROOM_EXIT.SendAsync();
        ...
        {
            Task<Response> task = Protocol.REQUEST_FF_EVENTROOM_JOIN.SendAsync();
            yield return Timing.WaitUntilTrue(task.IsComplete);
            ...
            Quest.QuestProgress(QuestType.FREE_FIGHT_ENTER_COUNT, FreeFightEventType.Christmas.ToIntValue()); // 특정 난전 도전 횟수
        ...
        protected override void OnReady()
        {
            ...
            SetSkillList();
            clonePlayer.Character.Initialize(player.Character);
            clonePlayer.Status.Initialize(player.Status);
            clonePlayer.Inventory.Initialize(player.Inventory.itemList.ToArray());
            ...
            uiBattlePlayerStatus.SetPlayer(clonePlayer);
            uiBattleMazeSkillList.SetCharacter(clonePlayer);
            uiBattleSkillList.SetCharacter(clonePlayer);
            clonePlayer.ResetSkillCooldown();
        ...
        private void RequestSkillMotion(UnitEntity target, SkillInfo skillInfo)
        {
            if (target is CharacterEntity characterEntity)
            {
                ...
                sfs.PutInt("1", clonePlayer.Character.Cid); // 공격자 아이디
        ...
        private void RequestSkill(UnitEntity[] targets, SkillInfo skillInfo)
        {
            ...
            switch (skillInfo.ActiveSkillType)
            {
                case ActiveSkill.Type.Attack:
                    ...
                    sfs.PutInt("6", skillInfo.SlotPos);
                    skillInfo.SetResponseCooldownCheckState(); // 쿨타임 대기 상태로 변경
        ...
        {
            savedPlayerPosition = clonePlayer.LastPosition;
        ...
        {
            UnitActor unitActor;
            bool isSpawnActor = impl.Add(clonePlayer, isEnemy: false);
            if (isSpawnActor)
            {
                clonePlayer.SetForceStatus(ForceStatusType.BasicStatusModeOn);
                clonePlayer.SetForceStatus(ForceStatusType.RefSkillOff);
                clonePlayer.SetExtraOption(ExtraBattleOptionType.MaxHp, maxHp); // MaxHp 맞추기
                clonePlayer.SetExtraOption(ExtraBattleOptionType.AttackSpeed, DEFAULT_SPEED);
                clonePlayer.SetExtraOption(ExtraBattleOptionType.MoveSpeed, DEFAULT_SPEED);
                clonePlayer.SetIgnoreWeapon(true);
                clonePlayer.SetIsFreeFightSkillCool(isFreeFightSkillCool: true);
                clonePlayer.SetForceStatus(ForceStatusType.BasicAttackSkillOff);

                clonePlayer.OnSpawnActor += OnCharacterSpawn;
                clonePlayer.OnDespawnActor += OnCharacterDespawn;

                unitActor = clonePlayer.SpawnActor(); // 유닛 소환
            ...
            {
                unitActor = clonePlayer.GetActor();
            }

            clonePlayer.SetIgnoreTarget(IsIgnoreTargetSettings()); // 타겟 무시 여부
            ...
            clonePlayer.SetCurrentHp(clonePlayer.MaxHP);
            clonePlayer.SetCurrentMp(clonePlayer.MaxMp);
        ...
        private bool SpawnPlayerBot(PlayerBotEntity entity, Vector3 pos, int maxHp, bool hasCurHp, int curHp, PlayerState state)
        {
            ...
            if (isSpawnActor)
            {
                entity.SetForceStatus(ForceStatusType.BasicStatusModeOn);
                entity.SetForceStatus(ForceStatusType.RefSkillOff);
                ...
                entity.SetIgnoreWeapon(true);
                entity.SetForceStatus(ForceStatusType.BasicAttackSkillOff);
            ...
            switch (state)
            {
                ...
                case PlayerState.Freeze:
                    unitActor.EffectPlayer.ShowFrozen(isPlaySfx: true);
                    break;
                ...
                case PlayerState.Shield:
                    unitActor.EffectPlayer.ShowShieldEffect();
                    break;

                default:
                    unitActor.EffectPlayer.HideFrozen();
                    unitActor.EffectPlayer.ReleasePowerUpEffect();
                    unitActor.EffectPlayer.ReleaseShieldEffect();
                    break;
            }
        ...
        private void RefreshRound()
        {
            ...
            switch (roundState)
            {
                ...
                case RoundState.End:
                    ...
                    uiFreeFightResult.Show(FreeFightEventType.Christmas, killCount, round, milliseconds);
                    break;
            }
        }
        ...
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            if (isPlayer)
            {
                // Do Nothing
                return;
            ...
            find.SetForceStatus(ForceStatusType.BasicStatusModeOff);
            find.SetForceStatus(ForceStatusType.RefSkillOn);
            ...
            find.SetIgnoreWeapon(false);
            find.SetForceStatus(ForceStatusType.BasicAttackSkillOn);
        ...
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            UnitEntity find = isPlayer ? clonePlayer : playerBotPool.Find(cid);
            if (find == null)
            {
                // 생성 도중이라면 생성큐 업데이트
        ...
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            bool isPlayerAttack = attackerCid == clonePlayer.Character.Cid;
            ...
            {
                attacker = isPlayerAttack ? clonePlayer : playerBotPool.Find(attackerCid);
            ...
            if (isPlayerAttack)
            {
                ...
                {
                    attacker.ForceStartCooldown(skillInfo);
        ...
        private void UpdatePlayerSkillMotion(int cid, int skillId, int targetCid)
        {
            ...
            bool isPlayerTarget = targetCid == clonePlayer.Character.Cid;
            UnitEntity target = isPlayerTarget ? clonePlayer : playerBotPool.Find(targetCid);
            if (target == null)
            {
                // 생성 도중
                if (playerBotPool.HasQueue())
        ...
        private void UpdatePlayerApplyBuff(int cid, int skillId, int[] targetCids, long slotId)
        {
            ...
            {
                bool isTargetPlayer = targetCid == clonePlayer.Character.Cid;
                UnitEntity target = isTargetPlayer ? clonePlayer : playerBotPool.Find(targetCid);
                if (target == null)
                {
                    // 생성 도중
                    if (playerBotPool.HasQueue())
                ...
                if (isPlayer)
                {
                    find.ForceStartCooldown(skillInfo);
                }
            }
        }
        ...
        private void UpdatePlayerPowerUpStart(int cid)
        {
            ...
            unitActor.EffectPlayer.ShowPowerUpEffect();
            unitActor.EffectPlayer.ReleaseShieldEffect(); // 파워업시 실드 제거
        }
        ...
        /// 플레이어 빙결 상태 시작
        /// </summary>
        private void UpdatePlayerFreezeStart(int cid)
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            UnitEntity find = isPlayer ? clonePlayer : playerBotPool.Find(cid);
            if (find == null)
            {
                const byte FREEZE_STATE = (byte)PlayerState.Freeze;

                // 생성 도중이라면 생성큐 업데이트
                if (playerBotPool.HasQueue() && playerBotPool.UpdateQueueState(cid, FREEZE_STATE))
                    return;

#if UNITY_EDITOR
                Debug.LogError($"존재하지 않는 플레이어가 빙결: {nameof(cid)} = {cid}");
#endif
                return;
            }
        ...
        /// 플레이어 빙결 상태 종료
        /// </summary>
        private void UpdatePlayerFreezeEnd(int cid)
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            UnitEntity find = isPlayer ? clonePlayer : playerBotPool.Find(cid);
            if (find == null)
...
#if UNITY_EDITOR
                Debug.LogError($"존재하지 않는 플레이어가 빙결 종료: {nameof(cid)} = {cid}");
#endif
                return;
            }
        ...
        /// 플레이어 실드 상태 시작
        /// </summary>
        private void UpdatePlayerShieldStart(int cid)
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            UnitEntity find = isPlayer ? clonePlayer : playerBotPool.Find(cid);
            if (find == null)
            {
                const byte SHIELD_STATE = (byte)PlayerState.Shield;

                // 생성 도중이라면 생성큐 업데이트
                if (playerBotPool.HasQueue() && playerBotPool.UpdateQueueState(cid, SHIELD_STATE))
                    return;

#if UNITY_EDITOR
                Debug.LogError($"존재하지 않는 플레이어가 실드: {nameof(cid)} = {cid}");
#endif
                return;
            }
            ...
            if (isPlayer)
            {
                SkillInfo skillInfo = FindSkill(find, GetSkillId(3));
        ...
        /// 플레이어 빙결 상태 종료
        /// </summary>
        private void UpdatePlayerShieldEnd(int cid)
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            UnitEntity find = isPlayer ? clonePlayer : playerBotPool.Find(cid);
            if (find == null)
...
#if UNITY_EDITOR
                Debug.LogError($"존재하지 않는 플레이어가 실드 종료: {nameof(cid)} = {cid}");
#endif
                return;
            }
        ...
        void OnStartController()
        {
            ...
            UnitActor unitActor = clonePlayer.GetActor();
            if (unitActor == null)
                return;
        ...
        void OnDragController(Vector2 position)
        {
            ...
            if (unitActor.IsPlayingActiveSkill()) // 액티브스킬 사용 중 이동 불가
                return;
            ...
            Vector3 dist = clonePlayer.LastPosition - savedPlayerPosition;
        ...
        void OnSelectSkill(SkillInfo info, UIBattleNormalSkillSlot.SlotType slotType)
        {
            ...
            switch (slotType)
            {
                case UIBattleNormalSkillSlot.SlotType.Skill:
                    ...
                    // 파워업 상태일때 실드 스킬 사용 불가
                    if (isPowerUp)
                    {
                        if (info.SkillId == GetSkillId(3))
                            return;
                    }

                    int mpCost = clonePlayer.CurMp - info.MpCost;
                    if (mpCost < 0)
                    {
        ...
        void OnUserFreezeStart(Response response)
        {
            int cid = response.GetInt("1");
            UpdatePlayerFreezeStart(cid);
        }

        void OnUserFreezeEnd(Response response)
        {
            int cid = response.GetInt("1");
            UpdatePlayerFreezeEnd(cid);
        }

        void OnUserShieldStart(Response response)
        {
            int cid = response.GetInt("1");
            UpdatePlayerShieldStart(cid);
        }

        void OnUserShieldEnd(Response response)
        {
            int cid = response.GetInt("1");
            UpdatePlayerShieldEnd(cid);
        }
        ...
        void SetSkillList()
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                if (data == null)
                {
                    Debug.LogError($"SkillData is Null: {nameof(skillId)} = {skillId}");
                }
                else
                {
                    ChristmasSkillData skill = new ChristmasSkillData();
                    skill.Initialize(data, slotIndex: i);
                    skillList.Add(skill);
                }
            }
        }

        int GetSkillId(int slot)
        {
            if (config.useSkills == null)
                return 0;

            return config.useSkills[slot];
        }

        void OnAttack(Response response)
        {
            if (response.ContainsKey("1"))
            {
                // 스킬 쿨타임 에러

                int skillId = response.GetInt("1");
                SkillInfo skillInfo = FindSkill(clonePlayer, skillId);
                ...
                clonePlayer.ForceStartCooldown(skillInfo);
            }
        }

        void OnActiveBuff(Response response)
        {
            if (response.ContainsKey("1"))
            {
                // 스킬 쿨타임 에러
