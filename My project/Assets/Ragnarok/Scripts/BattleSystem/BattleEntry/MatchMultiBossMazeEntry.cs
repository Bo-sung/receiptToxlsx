...
namespace Ragnarok
{
    public sealed class MatchMultiBossMazeEntry : BattleEntry
    {
        ...
        private const string TAG_SKILL_CAMERA = nameof(MatchMultiBossMazeEntry) + "_TAG_SKILL_CAMERA";
        ...
        public MatchMultiBossMazeEntry() : base(BattleMode.MatchMultiMazeBoss)
        {
#if SPAWN_AGENT_CHARACTER
        ...
        private async Task AsyncTimeOver()
        {
            ...
            // 나중에 (서버로 GiveUp 보내기 전에 서버에서 전투 종료가 날아올 수 있음)
            if (isShowResult)
                return;
        ...
        {
            MatchMultiBossMazeEntry.playerHp = playerHp;
        }
    }
}