...
namespace Ragnarok
{
    public sealed class WaterBombFreeFightEntry : BattleEntry
    {
        private const string SCENE_NAME = "TempSpecialMaze_Beach";
        ...
        public WaterBombFreeFightEntry() : base(BattleMode.WaterBombFreeFight)
        {
            ...
            config = FreeFightConfig.WATER_BOMB;

            playerBotPool = new PlayerBotEntityPool();
            battleTrapPool = new BattleTrapPool(BattleTrapType.WaterBombFreeFight);
        ...
        protected override void AddProtocalEvent()
        {
            ...
            Protocol.RECEIVE_FF_FREEZE.AddEvent(OnUserGroggyStart);
            Protocol.RECEIVE_FF_FREEZE_END.AddEvent(OnUserGroggyEnd);
        ...
        protected override void RemoveProtocolEvent()
        {
            ...
            Protocol.RECEIVE_FF_FREEZE.RemoveEvent(OnUserGroggyStart);
            Protocol.RECEIVE_FF_FREEZE_END.RemoveEvent(OnUserGroggyEnd);
        ...
        protected override void InitCanvas()
        {
            ...
            uiBattleFreeFight.SetFreeFightEventType(FreeFightEventType.WaterBomb);
        ...
        public override IEnumerator<float> YieldEnterBattle(int id)
        {
            ...
            Quest.QuestProgress(QuestType.FREE_FIGHT_ENTER_COUNT, FreeFightEventType.WaterBomb.ToIntValue()); // 특정 난전 도전 횟수
        ...
        private bool SpawnPlayerBot(PlayerBotEntity entity, Vector3 pos, int maxHp, bool hasCurHp, int curHp, PlayerState state)
        {
            ...
            switch (state)
            {
                ...
                case PlayerState.Groggy:
                    unitActor.EffectPlayer.ShowBubble(isPlaySfx: true);
                    break;
                ...
                default:
                    unitActor.EffectPlayer.HideBubble();
        ...
        private void RefreshRound()
        {
            ...
            switch (roundState)
            {
                ...
                case RoundState.End:
                    ...
                    uiFreeFightResult.Show(FreeFightEventType.WaterBomb, killCount, round, milliseconds);
                    break;
            }
        }
        ...
        /// 플레이어 그로기 상태 시작
        /// </summary>
        private void UpdatePlayerGroggyStart(int cid)
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            UnitEntity find = isPlayer ? clonePlayer : playerBotPool.Find(cid);
            if (find == null)
            {
                const byte GROGGY_STATE = (byte)PlayerState.Groggy;

                // 생성 도중이라면 생성큐 업데이트
                if (playerBotPool.HasQueue() && playerBotPool.UpdateQueueState(cid, GROGGY_STATE))
                    return;
        ...
        /// 플레이어 그로기 상태 종료
        /// </summary>
        private void UpdatePlayerGroggyEnd(int cid)
        {
            bool isPlayer = cid == clonePlayer.Character.Cid;
            UnitEntity find = isPlayer ? clonePlayer : playerBotPool.Find(cid);
            if (find == null)
        ...
        void OnUserGroggyStart(Response response)
        {
            int cid = response.GetInt("1");
            UpdatePlayerGroggyStart(cid);
        }

        void OnUserGroggyEnd(Response response)
        {
            int cid = response.GetInt("1");
            UpdatePlayerGroggyEnd(cid);
        }
