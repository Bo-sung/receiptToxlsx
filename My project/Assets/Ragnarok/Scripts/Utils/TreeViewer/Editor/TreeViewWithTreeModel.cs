...
namespace Ragnarok
{
    public class TreeViewWithTreeModel<T> : TreeView
        where T : TreeElement
    {
        ...
        void Search(T searchFromThis, string search, List<TreeViewItem> result)
        {
            if (string.IsNullOrEmpty(search))
                throw new System.ArgumentException("Invalid search: cannot be null or empty", "search");
            ...
            while (stack.Count > 0)
            {
                T current = stack.Pop();
                // Matches search?
                if (current.name.IndexOf(search, System.StringComparison.OrdinalIgnoreCase) >= 0)
                {
                ...
                case DragAndDropPosition.BetweenItems:
                    {
                        ...
                        if (args.performDrop && validDrag)
                        {
                            ...
                            OnDropDraggedElementsAtIndex(draggedRows, m_TreeModel.root, args.insertAtIndex == -1 ? 0 : args.insertAtIndex);
                        }
