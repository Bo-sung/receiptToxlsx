...
namespace Ragnarok
{
    public class PlayerCupetAI : CupetAI
    {
        protected override FSMSystem MakeFSM()
        {
            return new FSMSystem()
                ...
                .AddState(new ReadyState(actor, StateID.Ready)
                    ...
                    .AddTransition(Transition.Respawn, StateID.Respawn)) // 부활 => 부활
                ...
                .AddState(new IdleState(actor, StateID.Idle)
                    ...
                    .AddTransition(Transition.SawTarget, StateID.Chase) // AI종료 => 대기
                    .AddTransition(Transition.EndBattle, StateID.End)) // 전투 종료 => 종료 상태
                ...
                .AddState(new DieState(actor, StateID.Die)
                    .AddTransition(Transition.Respawn, StateID.Respawn) // 리스폰 => 리스폰 대기
                    .AddTransition(Transition.Rebirth, StateID.Rebirth) // 부활 => 부활
                    .AddTransition(Transition.EndBattle, StateID.End)) // 전투 종료 => 종료 상태

                /* 부활 */
                .AddState(new RebirthState(actor, StateID.Rebirth)
                    .AddTransition(Transition.Finished, StateID.FollowBindingTarget) // 상태(부활) 종료 => 플레이어 따라가기
                    .AddTransition(Transition.EndBattle, StateID.End)) // 전투 종료 => 종료 상태
                ...
                /* 리스폰 대기 */
                .AddState(new CupetRespawnState(actor, StateID.Respawn)
                    .AddTransition(Transition.Finished, StateID.Rebirth) // 리스폰 완료 => 부활
                    .AddTransition(Transition.EndBattle, StateID.End)); // 전투 종료 => 종료 상태
        }
    }
}