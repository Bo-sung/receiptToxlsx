...
public class TextureInfoItem
{
    ...
    public List<Material> MaterialObjList;
}
...
public class TextureInfoViewer : EditorWindow
{
    ...
    private void RefreshTextureInfoTables()
    {
        ...
        //List<Renderer> meshRenderers = FindObjectsOfTypeAll<Renderer>();
...
//            if (mr.isVisible)
        ...
        //}
        List<UnityEngine.UI.Image> imgs = FindObjectsOfTypeAll<UnityEngine.UI.Image>();
        foreach (UnityEngine.UI.Image rw in imgs)
        {
            AddVisibleTexture(rw.mainTexture, rw.material, rw.gameObject);
        }
        List<UnityEngine.UI.RawImage> rws = FindObjectsOfTypeAll<UnityEngine.UI.RawImage>();
        foreach (UnityEngine.UI.RawImage rw in rws)
        {
            AddVisibleTexture(rw.mainTexture, rw.material, rw.gameObject);
        }
        // NGUI
        //List<UIWidget> ngwgs = FindObjectsOfTypeAll<UIWidget>();
        UIWidget[] ngwgs = UnityEngine.Object.FindObjectsOfType (typeof(UIWidget)) as UIWidget[];
        foreach (UIWidget ngwg in ngwgs)
        {
            AddVisibleTexture(ngwg.mainTexture, ngwg.material, ngwg.gameObject);
        }
        //List<UIFont> ngfts = FindObjectsOfTypeAll<UIFont>();
        UIFont[] ngfts = UnityEngine.Object.FindObjectsOfType(typeof(UIFont)) as UIFont[];
        foreach (UIFont ngft in ngfts)
        {
            AddVisibleTexture(ngft.texture, ngft.material, ngft.gameObject);
        }
        ...
        foreach (var p in VisibleTextures)
        {
            ...
            si.MaterialObjList = new List<Material>(p.Value);
    ...
    Vector2 _selectTextureInfoScroll;
    TextureInfoItem _selectTexInfoItem;
    ...
    void OnGUI()
    {
        ...
        //float toolbar = 30.0f;
        //float padding = 10.0f;
        float toolbar = 30.0f, objbar = 50.0f;
        float padding = 2.0f;
        ...
        float categoryHeight = Mathf.Floor((position.height - toolbar - objbar) * 0.25f);
        ...
        float consumedHeight1 = toolbar + padding + categoryHeight;
        Rect r1 = new Rect(leftWidth + padding, consumedHeight1, rightWidth, categoryHeight);
        GUILayout.BeginArea(r1);
        if (_selectedTexture != null)
        {
            _selectTextureInfoScroll = GUILayout.BeginScrollView(_selectTextureInfoScroll, GUIStyle.none, GUI.skin.verticalScrollbar);
            if (_visibleGameObjects.ContainsKey(_selectedTexture))
            {
                var visibleGOs = _visibleGameObjects[_selectedTexture];
                for (int i = 0; i < visibleGOs.Count; ++i)
                    EditorGUILayout.ObjectField(visibleGOs[i], typeof(GameObject), false);
            }
            if (_selectTexInfoItem != null && _selectTexInfoItem.MaterialObjList != null)
            {
                for (int i = 0; i < _selectTexInfoItem.MaterialObjList.Count; ++i)
                    EditorGUILayout.ObjectField(_selectTexInfoItem.MaterialObjList[i], typeof(Material), false);
            }
            GUILayout.EndScrollView();
        }
        GUILayout.EndArea();
        //float consumedHeight = toolbar + padding + categoryHeight + padding + objbar + padding;	
        float consumedHeight = consumedHeight1 + objbar + categoryHeight;
